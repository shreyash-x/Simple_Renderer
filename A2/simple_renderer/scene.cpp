...
void Scene::intersectBVH(uint32_t nodeIdx, Ray &ray, Interaction &si, int interpolationVariant)
{
    BVHNode &node = this->nodes[nodeIdx];
    ...
    if (node.primCount != 0)
    {
        ...
        {
            Interaction siIntermediate = this->surfaces[this->getIdx(i + node.firstPrim)].rayIntersect(ray, interpolationVariant);
    ...
    {
        this->intersectBVH(node.left, ray, si, interpolationVariant);
        this->intersectBVH(node.right, ray, si, interpolationVariant);
    }
}

Interaction Scene::rayIntersect(Ray &ray, int interpolationVariant)
{
    Interaction si;
    si.didIntersect = false;

    this->intersectBVH(0, ray, si, interpolationVariant);
