...
void Surface::intersectBVH(uint32_t nodeIdx, Ray &ray, Interaction &si)
{
    ...
    if (node.primCount != 0)
    {
        ...
        for (uint32_t i = 0; i < node.primCount; i++)
        {
            ...
            {
                si = siIntermediate;
                si.uv = this->barycentricInterpolation(this->tris[this->getIdx(i + node.firstPrim)], si.p);
                ray.t = si.t;
            }
        }
    }
...
Vector2f Surface::barycentricInterpolation(Tri triangle, Vector3f p)
{
    Vector3f v21 = triangle.v2 - triangle.v1;
    Vector3f vp1 = p - triangle.v1;
    Vector3f v31 = triangle.v3 - triangle.v1;
    Vector3f vp2 = p - triangle.v2;
    Vector3f v32 = triangle.v3 - triangle.v2;

    // Total area of triangle
    float area = Cross(v21, v31).Length();
    // Calculate alpha
    float alpha = Cross(v32, vp2).Length() / area;
    // Calculate beta
    float beta = Cross(v31, vp1).Length() / area;
    // Calculate gamma
    float gamma = Cross(v21, vp1).Length() / area;

    Vector2f uv = triangle.uv1 * alpha + triangle.uv2 * beta + triangle.uv3 * gamma;
    return uv;
}

Vector3f Surface::fetchTexture(Vector2f uv, int interpolationVariant)
{
    if (this->hasDiffuseTexture())
    {
        if (interpolationVariant == 0)
        {
            // nearest neighbour
            return this->diffuseTexture.nearestNeighbourFetch(uv);
        }
        else if (interpolationVariant == 1)
        {
            // bilinear
            return this->diffuseTexture.bilinearFetch(uv);
        }
    }
    return this->diffuse;
}

Interaction Surface::rayIntersect(Ray &ray, int interpolationVariant)
{
    Interaction si;
    si.didIntersect = false;
    ...
    if (si.didIntersect)
    {
        si.color = this->fetchTexture(si.uv, interpolationVariant);
    }
    else
    {
        si.color = Vector3f(0.f, 0.f, 0.f);
    }
